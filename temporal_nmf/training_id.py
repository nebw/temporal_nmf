...
import temporal_nmf.model_id
from temporal_nmf.model_id import TemporalNMF
...
class TrainingWrapper:
    def __init__(
        ...
        self.age_embedder = temporal_nmf.model_id._default_age_embedder(num_hidden, num_factors).to(
            device
        )
        self.discriminator = temporal_nmf.model_id._default_discriminator(
            num_embeddings, num_hidden, self.num_classes
        ).to(device)

        self.offsetter = temporal_nmf.model_id._default_offsetter(
    ...
    def subbatch(
        ...
        label_offset,
        use_valid_ages=True,
        use_active_heuristic=True,
        zero_inflated_regression=True,
        preprocess=np.log1p,
    ):
        batch_idxs = torch.randint(0, data.num_entities, (batch_size,))

        batch_targets = preprocess(
            data.imls_log_daily[batch_idxs][:, batch_idxs]
            .transpose((2, 3, 0, 1))
            .astype(np.float32)
            .todense()
        )
        batch_targets = torch.from_numpy(batch_targets[:, :, :, :, None])
        ...
        loss_mask = 1 - torch.eye(batch_size)[None, None, :, :, None].repeat(
            data.num_days, data.num_timesteps, 1, 1, data.num_matrices
        )
        loss_mask = loss_mask.pin_memory().to(self.device, non_blocking=True)

        is_valid = np.ones(
            (data.num_days, data.num_timesteps, batch_size, batch_size), dtype=np.bool
        )

        if use_valid_ages:
            valid_ages = np.tile(
                data.valid_ages[: model.num_days, None, batch_idxs], (1, data.num_timesteps, 1)
            )
            is_valid &= valid_ages[:, :, None, :] * valid_ages[:, :, :, None]
            is_valid = torch.from_numpy(is_valid).pin_memory().to(self.device, non_blocking=True)

        if use_active_heuristic:
            heuristic_valid = batch_targets.sum(dim=(1, 3, 4)) > 0
            is_valid &= (
                heuristic_valid[:, None, :, None] * heuristic_valid[:, None, None, :]
            ).bool()

        loss_mask *= is_valid[:, :, :, :, None].float()

        if zero_inflated_regression:
            loss_mask[batch_targets == 0.0] *= (
                (batch_targets[is_valid[:, :, :, :, None]] > 0).float().mean().detach()
            )
