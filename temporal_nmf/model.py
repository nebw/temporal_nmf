...
class TemporalNMF(nn.Module):
    def __init__(self, num_entities, num_embeddings, num_factors, num_hidden, 
                 num_days, num_matrices, num_classes, ages, symmetric=True):
    ...
    def get_age_factors(self, idxs):
        ...
        factors_by_age = self.age_embedder(torch.clamp_min(ages.view(-1, 1), 0).to(self.get_device()))
    ...
    def get_embedding_factor_offsets(self, idxs):
        ...
        offsets = self.offsetter(
            ...
            dim=-1).view(-1, self.num_embeddings + 1)
    ...
    def reconstruction(self, factors):
        ...
        if symmetric:
            ...
            rec_unmapped = factors[:, :self.num_factors // 2, None] * factors[:, None, self.num_factors // 2:]

        rec_mapped = (rec_unmapped[:, :, :, None, :] @ output_map_reparam[None, None, None, :, :].to(rec_unmapped.device))[:, :, :, 0, :]
        
        return rec_mapped

    def reconstruct_inputs(self, with_offsets=True):
        with torch.no_grad():
            daily_bias_reshaped = self.daily_bias[:, None, None, :].repeat(1, self.num_entities, self.num_entities, 1)
            ...
            for day in trange(self.num_days):
                recs.append((self.reconstruction(factors_by_emb[day][None, :, :]) + daily_bias_reshaped[day]).cpu())
    ...
    def ortho_loss(self):
        return torch.sqrt(torch.mean(torch.pow((self.embeddings.weight.transpose(0, 1) @ self.embeddings.weight) - 
            (torch.eye(self.num_embeddings, dtype=torch.float32)).to(self.get_device()), 2)))

    @staticmethod
    def nonnegativity_loss(factors_by_age, factors_by_emb):
        return nn.functional.relu(-factors_by_emb).mean() + nn.functional.relu(-factors_by_age).mean()
    ...
    def get_factor_df(self, ids=None, embedding_dim=2, batch_size=128):
        if embedding_dim is not None:
            if embedding_dim == self.num_embeddings:
                ...
                embs_reduced = UMAP(n_components=embedding_dim).fit_transform(self.embeddings.weight.data.cpu().numpy())
        ...
        with torch.no_grad():
            ...
            while idx < self.num_entities:
                ...
                rec_by_age, rec_by_emb, factors_by_age, factors_by_emb, factor_offsets, embs = self.forward(batch_idxs)

                idx += batch_size

                bee_ages_flat = self.ages[:, batch_idxs].numpy().flatten()
                factors_flat = factors_by_emb.data.cpu().numpy().reshape(-1, self.num_factors)
                day_flat = np.tile(np.arange(self.num_days)[:, None], (1, len(batch_idxs))).flatten()
                columns = ['age', 'day'] + ['f_{}'.format(f) for f in range(self.num_factors)]
                df_data = np.concatenate((bee_ages_flat[:, None], day_flat[:, None], factors_flat), axis=-1) 
                ...
                if embedding_dim is not None:
                    ...
                    embs_flat = np.tile(embs_reduced[batch_idxs][None, :], (self.num_days, 1)).reshape(-1, embedding_dim)
...
class TemporalNMFWithTime(nn.Module):
    def __init__(self, num_entities, num_embeddings, num_factors, num_hidden, 
                 num_days, num_timesteps, num_matrices, num_classes, ages,
                 fix_output_map=False, symmetric=True):
        ...
        num_output_factors = self.num_factors #self.num_factors if symmetric else self.num_factors // 2
    ...
    def reconstruction(self, factors):
        ...
        factors = nn.functional.relu(factors[:, :, :, None, :]) * output_map_reparam[None, None, None, :, :]
        factors = torch.stack(factors.split(self.num_matrices, dim=-1), dim=-1)[:, :, :, 0, :, :]

        if self.symmetric:
            rec_mapped = (factors[:, :, :, None] * factors[:, :, None]).sum(dim=-1)
        else:
            rec_mapped = (factors[:, :, :, None, :, self.num_factors // 2:] * factors[:, :, None, :, :, self.num_factors // 2:]).sum(dim=-1)
    ...
    def timeembed_factors(self, factors):
        ...
        factors_with_time = torch.cat(
            (factors_repeated,
             times[None, :, None, :].repeat(self.num_days, 1, factors.shape[1], 1)), dim=-1)
    ...
    def forward(self, idxs):
        ...
        daily_bias_reshaped = self.daily_bias[:, None, None, None, :].repeat(1, self.num_timesteps, batch_size, batch_size, 1)
        
        factors_by_age = self.get_age_factors(idxs)
        factors_by_age_timeembedded, factors_by_age_timeembedded_offsets = self.timeembed_factors(factors_by_age)

        rec_by_age = self.reconstruction(factors_by_age_timeembedded) + daily_bias_reshaped

        embs, factor_offsets = self.get_embedding_factor_offsets(idxs)
        factors_by_emb = factors_by_age + factor_offsets
        factors_by_emb_timeembedded, factors_by_emb_timeembedded_offsets = self.timeembed_factors(factors_by_emb)

        rec_by_emb = self.reconstruction(factors_by_emb_timeembedded) + daily_bias_reshaped
        
        return rec_by_age, rec_by_emb, factors_by_age, factors_by_emb, \
            factors_by_age_timeembedded, factors_by_emb_timeembedded, \
                factors_by_age_timeembedded_offsets, factors_by_emb_timeembedded_offsets, \
                    factor_offsets, embs

    def get_factor_df(self, ids=None, embedding_dim=2, batch_size=128, importance_weighted_embeddings=True):
        if embedding_dim is not None:
            embs_reduced = self.embeddings.weight.data.cpu().numpy()
            ...
            if embedding_dim != self.num_embeddings:
                from umap import UMAP
                embs_reduced = UMAP(n_components=embedding_dim, min_dist=0).fit_transform(embs_reduced)
        ...
        with torch.no_grad():
            ...
            while idx < self.num_entities:
                batch_idxs = torch.arange(idx, min((idx + batch_size, self.num_entities)))
                rec_by_age, rec_by_emb, factors_by_age, factors_by_emb, _, _, _, _, factor_offsets, embs = self.forward(batch_idxs)
        ...
        with torch.no_grad():
            ...
            offsets_orig = self.offsetter(
                torch.cat((
                    ...
                    torch.clamp_min(ages[:, :, None, None], 0).repeat(1, 1, num_distortions, 1).to(device)),
                dim=-1).view(-1, self.num_embeddings + 1)
            )
            ...
            for emb_dim in range(self.num_embeddings):
                ...
                embs_distorted[:, :, emb_dim] = embs_distorted[:, :, emb_dim] + distortions[:, :, emb_dim]
