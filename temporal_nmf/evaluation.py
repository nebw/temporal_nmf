...
def load_circadian_df(curta_scratch_path):
    ...
    circadian_df["age"] = circadian_df[["bee_id", "date"]].apply(lambda f: get_bee_age(*f), axis=1)
...
def load_basics_df(path):
    ...
    basics = pd.DataFrame(
        [[b0] + list(b1) for b0, b1 in basics if b1 is not None],
        columns=[
            "datetime",
            "age",
            "bee_id",
    ...
    for target_var in tqdm(target_vars):
        ...
        if "circadian" in target_var:
            ...
            for _ in range(n_models):
                model = sm.Logit(
                    combined.reset_index()[target_var], features_scaled
                ).fit_regularized(alpha=1, disp=False, trim_mode="off", qc_tol=1)
                scores.append(
...
def evaluate_auc_over_time(combined, model_name, target_var="is_circadian"):
    ...
    combined_ = np.concatenate((combined[factor_names], combined[emb_names], day_onehot), axis=1)
    ...
    for day, group_indices in combined.groupby("day").indices.items():
        auc_score = metrics.roc_auc_score(
            combined.iloc[group_indices]["is_circadian"],
